        ...
        for step in range(self.steps):
            ...
            if last_adv_imgs is not None:
                ...
                for sample in samples:
                    adv_imgs_candidate = (sample[0]/100)*clone_adv_imgs+(sample[1]/100)*imgs+(sample[2]/100)*last_adv_imgs
                    adv_imgs_candidate.requires_grad_()
                    
                    adv_imgs_output = model.inference_image(self.normalization(adv_imgs_candidate))
                ...
                adv_imgs_for_grad = (samples[candidate_index][0]/100)*clone_adv_imgs+(samples[candidate_index][1]/100)*imgs+(samples[candidate_index][2]/100)*last_adv_imgs
            else:
                adv_imgs_for_grad = clone_adv_imgs

            adv_imgs_for_grad.requires_grad_()
            scaled_imgs = self.get_scaled_imgs(adv_imgs_for_grad, [0.5,0.75,1.25,1.5], device)
            ...
            with torch.enable_grad():
                ...
                for i in range(5):
                    ...
                    loss += loss_item
            adv_imgs_for_grad.retain_grad()
            loss.backward()
            grad = adv_imgs_for_grad.grad
            
            # MDCS-MI
            beta_t = self.beta
            norm_grad = grad / (torch.mean(torch.abs(grad), dim=(1,2,3), keepdim=True) + 1e-12)
            momentum = beta_t * momentum + norm_grad
