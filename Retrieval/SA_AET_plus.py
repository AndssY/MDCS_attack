...
class ImageAttacker():
    ...
    def txt_guided_attack(self, model, imgs, txt2img, all_txt_supervisions, device, scales=None, txt_embeds=None):
        ...
        b = imgs.shape[0]
        scales_list = scales if scales is not None else []
        scales_num = len(scales_list) + 1

        # Initialize adversarial images and MDCS variables
        ...
        last_adv_imgs = adv_imgs.clone().detach() # Initialize last_adv_imgs for the first step's logic
        momentum = torch.zeros_like(imgs, device=device)
        d_t = torch.ones_like(imgs, device=device) * float('inf')
        delta = (adv_imgs - imgs).detach().requires_grad_(True)

        start_time = time.time()
        
        for step in range(self.steps):
            clone_adv_imgs = adv_imgs.clone().detach()
            ...
            if step > 0:
                ...
                for sample in samples:
                    candidate_imgs = (sample[0] / 100) * clone_adv_imgs + \
                                     (sample[1] / 100) * imgs + \
                                     (sample[2] / 100) * last_adv_imgs
                    
                    candidate_imgs.requires_grad = True

                    # Evaluate the candidate by simulating a PGD step
                    if self.normalization:
                        candidate_output = model.inference_image(self.normalization(candidate_imgs))
                    else:
                        candidate_output = model.inference_image(candidate_imgs)
                    
                    model.zero_grad()
                    with torch.enable_grad():
                        loss_eval = self.loss_func(candidate_output['image_feat'], txt_embeds, txt2img, all_txt_supervisions)
                    
                    candidate_imgs.retain_grad()
                    loss_eval.backward()
                    
                    grad_eval = candidate_imgs.grad
                    perturbation_eval = self.step_size * grad_eval.sign()
                    
                    temp_adv_imgs = clone_adv_imgs + perturbation_eval
                    temp_adv_imgs = torch.min(torch.max(temp_adv_imgs, imgs - self.eps), imgs + self.eps)
                    temp_adv_imgs = torch.clamp(temp_adv_imgs, 0.0, 1.0)

                    with torch.no_grad():
                        if self.normalization:
                            final_output = model.inference_image(self.normalization(temp_adv_imgs))
                        else:
                            final_output = model.inference_image(temp_adv_imgs)
                        final_loss = self.loss_func(final_output['image_feat'], txt_embeds, txt2img, all_txt_supervisions)
                        loss_list.append(final_loss.item())

                    del candidate_imgs, candidate_output, loss_eval, grad_eval, perturbation_eval, temp_adv_imgs, final_output, final_loss
                
                best_sample = samples[loss_list.index(max(loss_list))]
                adv_imgs_for_update = (best_sample[0] / 100) * clone_adv_imgs + \
                                      (best_sample[1] / 100) * imgs + \
                                      (best_sample[2] / 100) * last_adv_imgs
            else:
                adv_imgs_for_update = adv_imgs.clone()

            last_adv_imgs = clone_adv_imgs

            adv_imgs_for_update.requires_grad = True
            scaled_imgs = self.get_scaled_imgs(adv_imgs_for_update, scales_list, device)
            ...
            model.zero_grad()
            with torch.enable_grad():
                loss = sum(self.loss_func(adv_imgs_embeds[i*b : (i+1)*b], txt_embeds, txt2img, all_txt_supervisions) for i in range(scales_num))
            
            adv_imgs_for_update.retain_grad()
            loss.backward()
            grad = adv_imgs_for_update.grad

            del scaled_imgs, adv_imgs_output, adv_imgs_embeds, adv_imgs_for_update
            
            # === MDCS ===
            norm_grad = grad / (torch.mean(torch.abs(grad), dim=(1, 2, 3), keepdim=True) + 1e-12)
            momentum = self.beta * momentum + norm_grad
            d_t = torch.min(self.gamma / (torch.abs(momentum) + 1e-12), d_t)
            
            inner_term = delta + self.step_size * d_t * momentum + self.step_size * self.decay_factor * delta.detach()
            new_delta = torch.pow(d_t, -0.5) * inner_term
            
            adv_imgs = imgs + new_delta
        ...
        end_time = time.time()
        print(f"The function execution time: {time.time() - start_time:.2f} seconds")
